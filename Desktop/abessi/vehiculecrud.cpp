...
bool VehiculeCRUD::validerChamps(QString &messageErreur)
{
    ...
    if (type.trimmed().isEmpty()) {
        erreurs << "Type de véhicule obligatoire.";
    }

    // Statut obligatoire
    if (statut.trimmed().isEmpty()) {
        erreurs << "Statut du véhicule obligatoire.";
    }

    // Heure de départ : obligatoire et seulement des chiffres
    if (heureDepart.trimmed().isEmpty()) {
        erreurs << "Heure de départ obligatoire.";
    } else {
        QRegularExpression rxHeure("^[0-9]+$");
        if (!rxHeure.match(heureDepart).hasMatch()) {
            erreurs << "Heure de départ doit contenir uniquement des chiffres.";
        }
    }

    // Heure d'arrivée : obligatoire et seulement des chiffres
    if (heureArrivee.trimmed().isEmpty()) {
        erreurs << "Heure d'arrivée obligatoire.";
    } else {
        QRegularExpression rxHeure("^[0-9]+$");
        if (!rxHeure.match(heureArrivee).hasMatch()) {
